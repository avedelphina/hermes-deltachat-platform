...
- **Bot-loop guard**: `DELTACHAT_MAX_CONSECUTIVE_REPLIES` (default 20, `<=0` disables) stops the adapter from processing further messages from the same sender in a chat once they've sent that many in a row with no other participant chiming in — guards against two or more bots (e.g. multiple Hermes+Delta Chat agents in one group) auto-replying to each other forever. Sends one notice (subject to `DELTACHAT_SEND_REJECTION_REPLIES`) the first time a streak trips; resets as soon as anyone else speaks.
