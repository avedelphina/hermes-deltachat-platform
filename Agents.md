...
- **Mention matching tolerates case-ending variation** (`_build_mention_pattern` in `adapter.py`): built for Czech declension (e.g. `display_name: Alice` also matches "Alici"/"Alicí"; "Anikke" also matches the incorrectly-declined "Anikko"), implemented generically as stem + up to 2 trailing word characters, not a Czech-specific grammar table. The stem must be ≥3 chars — shorter names (e.g. "Tom") fall back to an exact match instead, to avoid matching unrelated words that happen to share a short prefix. `DELTACHAT_MENTION_ALIASES` (comma-separated) adds extra exact-ish forms (each still gets the same stem-tolerant treatment) for names/nicknames the automatic stemming doesn't cover.
